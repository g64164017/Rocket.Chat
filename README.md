...
```bash
openssl genrsa 1024 > certificate.key
chmod 400 certificate.key
openssl req -new -x509 -nodes -sha1 -days 365 -key certificate.key -out certificate.cert
sudo cp certificate* /etc/nginx/
```
...
```
> Pada konfigurasi ini, kedua _service_ akan berjalan otomatis setiap kali _boot_.
